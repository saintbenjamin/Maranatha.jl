...
```math
\\int\\limits_{a}^{b} \\int\\limits_{a}^{b} \\; dx dy \\; f(x,y)
```
...
```math
I_x \\approx \\int dy \\; \\left[ \\partial_x^{\\texttt{m}} f(x_L,y) - \\partial_x^{\\texttt{m}} f(x_R,y) \\right]
```
- Along the ``y`` axis (integrate over ``x``):
```math
I_y \\approx \\int dx \\; \\left[ \\partial_y^{\\texttt{m}} f(x,y_L) - \\partial_y^{\\texttt{m}} f(x,y_R) \\right]
```
and returns
```math
E \\approx \\texttt{K} \\, h^\\texttt{p} \\, ( I_x + I_y ) \\,
```
where
- ``\\displaystyle{h = \\frac{b-a}{N}}``,
- ``x_L = a + \\texttt{z} \\, h``, ``x_R = a + ( N - \\texttt{z} ) \\, h`` (and similarly ``y_L``, ``y_R``),
...
3) Approximate two axis-wise contributions:
   - ``\\displaystyle{I_x = \\int dy \\; \\partial_x^{\\texttt{m}} f(\\bar{x},y)}`` by sampling ``\\partial_x^{\\texttt{m}}f`` at ``\\bar{x}`` and integrating over ``y``,   
   - ``\\displaystyle{I_y = \\int dx \\; \\partial_y^{\\texttt{m}} f(x,\\bar{y})}`` by sampling ``\\partial_y^{\\texttt{m}}f`` at ``\\bar{y}`` and integrating over ``x``.
4) Return ``E \\approx C \\; (b-a) \\; h^{\\texttt{m}} \\; (I_x + I_y)``.
